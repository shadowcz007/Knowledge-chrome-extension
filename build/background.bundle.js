...
async function queryNotionDataBase(token, databaseId, keywordsSetup) {
  ...
  try {
    ...
    if (error.code === APIErrorCode.ObjectNotFound) {} else {
      console.error(error);
    }
    info = error;
  }
...
chrome.runtime.onMessage.addListener(async function (request, sender, sendResponse) {
  ...
  if (cmd == 'mark-run') {
    ...
    queryNotionDataBase(token, databaseId, keywordsSetup).then(({
      result,
      success,
      info
    }) => {
    ...
    if (request.isMy && request.address) {
      // 我的贡献
      queryByCfxAddress(request.address, start_cursor, page_size).then(({
        result,
        success,
        info
      }) => {
    ...
    getAllTags(startCursor, pageSize).then(({
      result,
      success,
      info
    }) => {
...
(function () {
  ...
  reactHotLoader.register(chromeStorageGet, "chromeStorageGet", "C:\\Users\\38957\\Documents\\GitHub\\Knowledge-chrome-extension\\src\\pages\\Background\\index.js");
...
/******/ 	(() => {
/******/ 		__webpack_require__.h = () => ("0452b6b3e07e5ee0a86a")
/******/ 	})();
/******/ 	
