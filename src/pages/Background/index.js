...
const { notionToken: token, notionDatabaseId: databaseId } = chrome.runtime.getManifest().config;
...
chrome.contextMenus.onClicked.addListener(async(item, tab) => {
    ...
    if (id == 'keyword') {
        ...
        if (!cfxAddress) {
            ...
            // 通知页面进行标注
            chrome.tabs.sendMessage(tab.id, { cmd: 'login', tabId: tab.id }, function(response) {
                console.log(response)
            })
        } else {
            // 通知页面进行标注
            chrome.tabs.sendMessage(tab.id, { cmd: 'mark-run', tabId: tab.id }, function(response) {
                console.log(response)
            })
        }
    }
        ...
        if (!cfxAddress) {
            ...
            save({...request.data, cfxAddress }).then((res) => {
                ...
                chrome.tabs.query({ active: true, currentWindow: true },
                    function(tabs) {
                        chrome.tabs.sendMessage(
                            tabs[0].id, { cmd: 'mark-push', data: parseData(res) },
                            function(response) {
                                console.log(response.farewell)
                            }
                        )
